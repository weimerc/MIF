...
to go
  ...
  if not SWN? [ update-clusters ]
  if not SWN? [ if max map [ [y] -> max [opinion] of y - min [opinion] of y ] clusters <= (group-width / 2) [stop] ]
  if count turtles with [opinion >= 0.0001 and opinion <= 0.9999] = 0 [ stop ]
  if max [opinion] of turtles - min [opinion] of turtles < 0.01 [ stop ]
  if SWN? and ticks > 0 and ticks mod 50 = 0 [ if max [ abs (get-influence get-ops)] of turtles with [opinion > 0 and opinion < 1] <= (group-width / 100) [ stop ] ]
end
...
to-report prototypicality [ x ops ] ; P(x,X) in Salzarulo (2006), modified with lambda multiplier
  ...
  let P (a * lambda * dinter - (1 - a) * dintra)
  report P
end

to-report P-deriv [ x ops ] ; (d/dx)P(x,X) as specified above, with lambda multiplier
  let a outgroup-aversion
  let w group-width
  let mus map [[xi] -> membership x xi] ops
  ...
  let diff2mus2 sum map [[y] -> (1 - item y mus) * (item y diffs2)] range length ops; sum((x-xi)^2 (1-mu(x,xi)))
  let ddintradx ifelse-value (summus = 0) [0] [2 * (((((w ^ 2) * diffmus) - diff3mus) / ((w ^ 2) * summus)) + ((diff2mus * diffmus) / ((w * summus) ^ 2)))]
  let ddinterdx ifelse-value (summus2 = 0) [0] [2 * (((((w ^ 2) * diffmus2) + diff3mus) / ((w ^ 2) * summus2)) - ((diff2mus2 * diffmus) / ((w * summus2) ^ 2)))]
  let lambda (w ^ 2) / ((exp 1) - (exp (1 - (1 / (w ^ 2)))))
  let dPdx (a * lambda * ddinterdx - (1 - a) * ddintradx)
  report dPdx
end

to-report influence-MIF [ x ops ] ; converts P-deriv to actual influence value
  let influence P-deriv x ops
  set influence k * influence
  report influence
end

to-report norm-var ; report normalized variance of opinions
  let var variance [opinion] of turtles ; NetLogo calculates the sample variance (not population)
  report 12 * var * (N - 1) / N ; Convert to pop variance, divide by 1/12
end
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot norm-var"
...
@#$#@#$#@
<experiments>
  <experiment name="BaseExperiment" repetitions="100" sequentialRunOrder="false" runMetricsEveryStep="false">
    <setup>setup
reset-timer</setup>
    <go>go</go>
    <metric>timer</metric>
    <metric>mean [opinion] of turtles</metric>
    <metric>norm-var</metric>
    <metric>cluster-ops</metric>
    <metric>length clusters</metric>
    <metric>map [[y] -&gt; count y] clusters</metric>
  ...
  <experiment name="SWNExperiment" repetitions="100" sequentialRunOrder="false" runMetricsEveryStep="false">
    <setup>setup
reset-timer</setup>
    <go>go</go>
    <metric>timer</metric>
